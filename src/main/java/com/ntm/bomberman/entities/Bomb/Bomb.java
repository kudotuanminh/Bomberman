...
import com.ntm.bomberman.entities.objects.Wall;
...
public class Bomb extends Entity {
    private int timeToExplodes = 80; // 100 seconds
    private int xLeft;
    private int xRight;
    private int yTop;
    private int yBottom;
    ...
    private void addExplosion() {
        if (!(entity_left instanceof Wall)) {
            explosions.add(new Explosion(xLeft / 32 , y / 32 , Sprites.explosion_horizontal_2.getFxImage()));
        }
        if (!(entity_right instanceof Wall)) {
            explosions.add(new Explosion(xRight / 32 , y / 32 , Sprites.explosion_horizontal_2.getFxImage()));
        }
        if (!(entity_up instanceof Wall)) {
            explosions.add(new Explosion(x / 32 , yTop / 32 , Sprites.explosion_vertical_2.getFxImage()));
        }
        if (!(entity_down instanceof Wall)) {
            explosions.add(new Explosion(x / 32 , yBottom / 32 , Sprites.explosion_vertical_2.getFxImage()));
        }
        explosions.add(new Explosion(x / 32 , y / 32 , Sprites.bomb_exploded_2.getFxImage()));
    }
}
